...
const products_reducer = (state, action) => {
  ...
  if(action.type === GET_PRODUCTS_SUCCESS){
    const featuredProducts =  action.payload.filter((product)=> { return product.featured === true})
    const products = action.payload
    return { ...state, products_loading: false, products: products, featuredProducts: featuredProducts}
  }
